...
const Login = ({navigation}) => {
  ...
  const signUp = async () => {
    ...
    if (!validateEmail(email)) {
      SimpleToast.show("Please provide a valid email address", SimpleToast.SHORT);
      return;
    }
    
    if (password.length < 6) {
      SimpleToast.show("Password should be at least 6 characters long", SimpleToast.SHORT);
      return;
    }
    ...
    try {
      ...
      await firestore().collection("chats").doc(user.uid).set({
        chatsData: []
      }).then(() => { console.log("User's chats created!") })
      ...
      setLoading(false)
      SimpleToast.show("Error creating account: " + err.message, SimpleToast.LONG);
      console.log(err)
    }
  }
  ...
  const signIn = async () => {
    ...
    try {
      ...
      setLoading(false)
      SimpleToast.show(err.code.split("/")[1].split("-").join(" "), SimpleToast.SHORT);
      console.log(err)
        ...
        currState === "Sign up"
          ? (
            <View style={styles.innerContainer}>
              ...
              <TextInput style={styles.input} value={username} onChangeText={setUsername} placeholder='Username..' placeholderTextColor="gray" editable={!loading}/>
              <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder='Email..' placeholderTextColor="gray" editable={!loading}/>
              <TextInput style={styles.input} secureTextEntry value={password} onChangeText={setPassword} placeholder='Password..' placeholderTextColor="gray" editable={!loading}/>
              <Button title={currState === 'Sign up' ? 'Create account' : 'Login now'} onPress={signUp} disabled={loading}/>
          ...
          : (
            <View style={styles.innerContainer}>
              ...
              <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder='Email..' placeholderTextColor="gray" editable={!loading}/>
              <TextInput style={styles.input} secureTextEntry value={password} onChangeText={setPassword} placeholder='Password..' placeholderTextColor="gray" editable={!loading}/>
              <Button title={currState === 'Sign up' ? 'Create account' : 'Login now'} onPress={signIn} disabled={loading}/>
              <Text style={styles.switch}>
                Already have an account?{' '}
