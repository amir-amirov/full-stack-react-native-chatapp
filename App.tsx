...
function Chat({ navigation }) {
  ...
  return (
    <ChatStack.Navigator initialRouteName='ChatList'>
      ...
      <ChatStack.Screen name="ChatBox" component={ChatBox} options={{ headerShown: false }} />
    </ChatStack.Navigator>
  );
}
      ...
      if (user) {
        try {
          ...
          if (userData) {  // Ensure userData is not undefined
            ...
            console.error("No user data found for this user! Ignore, If raised after signup");
          }
      ...
      {isAuthenticated ? (
        <Tab.Navigator
          screenOptions={({ route }) => ({
            ...
            tabBarLabel: ({ focused }) => (
              <Text style={{ fontSize: focused ? 15 : 14, fontWeight: 'bold', color: "gray" }}>
                {route.name}
              </Text>
            ),
          })}
